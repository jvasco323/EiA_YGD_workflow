...
args <- list(par1=30, par2=5)  # 30=wheat; 5=Ethiopia
args <- lapply(args, function(x) format(x, scientific = FALSE))
parameters <- paste0(names(args), "=", args, collapse = "&")
request <- httr::GET(paste0("https://www.yieldgap.org/apigyga/json/cropcountrystationyear", "?", 
                            parameters),
                     httr::add_headers(`x-gyga-apikey`="a2183f47-cc6c-39cd-dd25-d9fa974b0d2f"))
...
The next step entails merging the yield data from GYGA with the respective weather station coordinates. Once data are merged, the crop yield data is made spatially explicit using the *st_as_sf()* function. Please refer to the chunk of code below.
...
data_rfwh_with_data <- subset(data_rfwh, Yw_2017 >= 0) # arbitrary year of the data
...
data_rfwh_no_data <- subset(data_rfwh, Yw_2017 < 0) # arbitrary year of the data
...
args <- list(par1=30, par2=5)
args <- lapply(args, function(x) format(x, scientific=FALSE))
parameters <- paste0(names(args), "=",  args, collapse = "&")
request <- httr::GET(paste0("https://www.yieldgap.org/apigyga/json/cropcountryyear" , "?", 
                            parameters),
                     httr::add_headers(`x-gyga-apikey`="a2183f47-cc6c-39cd-dd25-d9fa974b0d2f"))
